...
def kernel_IntGauss1D(ii, x, sigma) -> float:
    '''_summary_
...
def kernel_DerivativeIntGauss1D(ii, x, sigma, N, PSFy, calc_d2):
    ...
    dudt = - (N / (math.sqrt(2.0 * np.pi) * sigma)) * (a-b) * PSFy
    d2udt2 = 0.0

    if (calc_d2):
        d2udt2 = - (N / (math.sqrt(2.0 * np.pi) * math.pow(sigma, 3))) * \
...
def kernel_DerivativeIntGauss1DSigma(ii, x, Sx, N, PSFy, calc_d2):
    ...
    dudt = - (N/(math.sqrt(2*np.pi)*Sx*Sx)) * \
    ...
    if (calc_d2):
        d2udt2 = (
            - (2 / Sx)*dudt
            - (N/(math.sqrt(2*np.pi)*math.pow(Sx, 5)))*(
...
@cuda.jit(
    nb.types.Tuple((nb.float32, nb.float32))(
        nb.int32, nb.float32, nb.float32[:]),
    device=True)
def kernel_GaussFMaxMin2D(sz, sigma, data) -> tuple[float, float]:
...
def kernel_MatInvN(M, Minv, DiagMinv, sz):
    ...
    '''
    tmp1 = 0.0
    yy = cuda.local.array(shape=30, dtype=np.float32)
