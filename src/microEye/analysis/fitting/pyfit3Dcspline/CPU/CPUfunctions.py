...
@nb.njit(
    nb.types.Tuple((nb.float32, nb.float32))(
        nb.int32, nb.float32, nb.float32, nb.float32, nb.float32, nb.boolean
    ),
    cache=True,
)
def kernel_DerivativeIntGauss1D(ii, x, sigma, N, PSFy, calc_d2):
    ...
    if calc_d2:
        d2udt2 = (
            -(N / (math.sqrt(2.0 * pi) * math.pow(sigma, 3)))
            * ((ii + 0.5 - x) * a - (ii - 0.5 - x) * b)
            * PSFy
        )
...
def kernel_DerivativeIntGauss1DSigma(ii, x, Sx, N, PSFy, calc_d2):
    ...
    dudt = (
        -(N / (math.sqrt(2 * pi) * Sx * Sx))
        * (ax * (ii - x + 0.5) - bx * (ii - x - 0.5))
        * PSFy
    )
    d2udt2 = 0
    ...
    if calc_d2:
        d2udt2 = (
            ...
            * (ax * math.pow((ii - x + 0.5), 3) - bx * math.pow((ii - x - 0.5), 3))
            * PSFy
        )
...
def kernel_DerivativeIntGauss2DSigma(ii, jj, x, y, S, N, PSFx, PSFy, calc_d2):
    ...
    dSx, ddSx = kernel_DerivativeIntGauss1DSigma(ii, x, S, N, PSFy, calc_d2)
    dSy, ddSy = kernel_DerivativeIntGauss1DSigma(jj, y, S, N, PSFx, calc_d2)
...
@nb.njit(nb.types.Tuple((nb.float32, nb.float32))(nb.int64, nb.float32[:]), cache=True)
...
@nb.njit(
    nb.types.Tuple((nb.float32, nb.float32))(nb.int32, nb.float32, nb.float32[:]),
    cache=True,
)
...
@nb.njit(
    nb.types.Tuple((nb.float32[:], nb.float32))(
        nb.int32, nb.int32, nb.float32, nb.float32[:], nb.float32[:]
    ),
    cache=True,
)
...
def kernel_DerivativeIntGauss2Dz(
    ...
    dudt[0], ddx = kernel_DerivativeIntGauss1D(ii, theta[0], Sx, theta[2], PSFy, True)
    dudt[1], ddy = kernel_DerivativeIntGauss1D(jj, theta[1], Sy, theta[2], PSFx, True)
    dSx, ddSx = kernel_DerivativeIntGauss1DSigma(ii, theta[0], Sx, theta[2], PSFy, True)
    dSy, ddSy = kernel_DerivativeIntGauss1DSigma(jj, theta[1], Sy, theta[2], PSFx, True)
    ...
    if calc_d2:
        ...
        ddSddzx = d2Sdalpha2_x * math.pow(
            kernel_dalphadz(z - gamma, Ax, Bx, d), 2
        ) + dSdalpha_x * kernel_d2alphadz2(z - gamma, Ax, Bx, d)
        ddSddzy = d2Sdalpha2_y * math.pow(
            kernel_dalphadz(z + gamma, Ay, By, d), 2
